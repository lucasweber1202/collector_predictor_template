...
from scripts.time_series import Observation, WriteResult, upsert_time_series
...
def test_current_mutable_file_backfill_is_first_seen() -> None:
    ...
    result = WriteResult(
        1, 0, [(SERIES, date(2020, 1, 1), collected.date())], frozenset(), frozenset()
    )
    data = SimpleNamespace(
        observations=[
            SimpleNamespace(series_id=SERIES, reference_date=date(2020, 1, 1), snapshot_id="s")
        ],
...
def test_legacy_official_timestamp_cannot_predate_collection(engine: Engine) -> None:
    ...
    with engine.begin() as conn:
        written = upsert_time_series(conn, [Observation(SERIES, reference, 100.0, "s")], collected)
        upsert_availability(
            conn,
            [
                {
